const nameCheckbox = document.querySelectorAll("#filter-name-options input[type=checkbox]")
const ratingCheckbox = document.querySelectorAll("#filter-rating-options input[type=checkbox]")
const typeCheckbox = document.querySelectorAll("#filter-type-options input[type=checkbox]")
...
function filterCoffeesby(field,terms) {
    users.forEach(user => {
        ...
        if(field == "rating") {
            ...
            value = Array.from(typeSelect.options).map(opt => opt.value.toLowerCase())
        }
        else {
